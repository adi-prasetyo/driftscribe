...
- [`workers/`](workers/) — 実行専用のワーカーサービス: ワークロード別 2 ペア (drift `reader` / `docs` / `rollback`、upgrade `upgrade-reader` / `upgrade-docs`) と共有の `notifier`
...
Phase 17 (マルチエージェントフレームワーク + Developer Knowledge MCP) を進行中です。
