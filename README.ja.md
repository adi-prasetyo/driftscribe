...
| エージェントチーム | トリガー | 担当範囲 | 安全境界 |
| --- | --- | --- | --- |
| **Anchor** | 自律 (Eventarc) | 実環境の Cloud Run 設定と ops コントラクトの突合 → docs PR、ドリフト issue、またはロールバック | ロールバックは、一回限り有効な人による確認・承認 (HITL) が完了するまで実行されない |
| **Patch** | オンデマンド (チャット) | npm 依存パッケージと GitHub Advisory DB の突合 → upgrade PR | メジャーバンプは決定論的バリデータが拒否 |
| **Provision** | オンデマンド (チャット) | `iac/` のみを変更する OpenTofu PR を作成 | 稼働中のインフラには触れず、適用は別のゲート付きワーカーが担う |
| **Explore** | オンデマンド (チャット) | プロジェクト全体の読み取り専用インベントリ。DriftScribe 自体の仕組みも説明する | ミューテーションツールはゼロ (テストで保証) |

**エージェントチームの連携:** 全体はループとして動きます。Provision が新しいインフラを
立ち上げ（依頼すると IaC PR を開きます）、続いて Anchor が稼働中のインフラを
...
された制約の中で動く単一目的のワーカーが担い、ロールバックと IaC の適用は必ず
一回限り有効な人間の承認ゲートで待ち、すべての判断は推論トレース
...
### Anchor — Cloud Run の設定ドリフト (`drift`)

自律的に動作します。監視対象の `payment-demo` Cloud Run サービスの config 変更に反応する Eventarc トリガーが組み込まれており（ポーリングループではなくイベント駆動）、チャットから呼び出す必要はありません。
...
- 稼働中のインフラには一切触れません。実際の `tofu apply` は**下流**の `tofu-apply` ワーカー (稼働中のインフラに対して `tofu apply` を実行する唯一のもの) が、プランに紐付いた HMAC 署名付きオペレーター承認の背後で実行します。これはチャットエージェントが直接呼び出すことはできない経路です。

### Explore — 読み取り専用の調査 (`explore`)

オンデマンド、チャット専用です (`/recheck` はこちらも拒否します)。

- 4 つのエージェントチームの中で最も広い読み取り範囲を持ちます。実環境の Cloud Run 環境変数、ops コントラクト、依存パッケージの lockfile、Cloud Asset Inventory (`infra-reader` ワーカー) によるプロジェクト全体のリソースインベントリ、保留中の IaC プランアーティファクト、判断履歴、過去の会話、そして開発者ドキュメントまで、すべてのレーンを横断して参照します。
- DriftScribe 自体の仕組みを説明する役割も担います。プロンプトにシステム全体の概要を持つため、初めての人もドキュメントを読む前にチャットで全体像をつかめます。他の 3 つのエージェントチームは「DriftScribe はどう動くのか」という質問をここに誘導します。
- ミューテーションツールはゼロ。すべてを読めて、何も変更できません (この読み取り専用保証は、ツール集合がミューテーション集合と交わらないことを検証するテストで保証されています)。

オペレーター UI は、判断履歴と並べて実環境のインフラリソースマップ (IaC 管理済み vs. 取り込み対象) を表示します。

**「ドリフト」が指すのは設定ドリフトだけです。** Anchor のドリフトは、`payment-demo` サービス 1 つを対象に実環境の環境変数を ops コントラクトと突合するもので、Eventarc が数秒以内に検知します。ops コントラクトで宣言されていたものが実際にずれた、という意味でのドリフトです。リソースマップが答えるのは別の問いなので、言葉も分けています。`iac/` に記述されていないプロジェクト内の実リソースは、そもそも IaC に定義されたことがなく、ずれようがありません。そのため UI はこれをドリフトとは呼ばず、そのうち実際に取り込めるものを**取り込み対象**と呼んでいます。これは `infra-reader` ワーカーが Cloud Asset Inventory 経由で検出し、Anchor ではなく IaC 管理への取り込み (Provision がインポート PR を作成) で解消します。Cloud Asset Inventory は結果整合性のインデックスで、リソースマップはその上にキャッシュされるため、手動で作成したリソースがマップに現れるまで数分かかります。これは検出のレイテンシであり、不具合ではありません。

**読み取り範囲の一覧。** エージェントチームは「できること」だけでなく「見えるもの」でも異なります。Anchor と Patch は自分のレーンしか読みません。Provision は変更を書くために必要なインフラインベントリを加えます。Explore はすべてのレーンを横断して読み、それがオリエンテーション役たるゆえんです。
...
† Explore は最近の PR 検索を意図的に持ちません。このツールは書き込み可能な GitHub トークンを使うため、厳密に読み取り専用である Explore が保持してはならないからです。Explore の他のツールはすべて読み取り専用の資格情報で動きます。

## デモ

デモの本体は、<https://driftscribe.adp-app.com> で公開しているライブのオペレーター
UI です。インフラのリソースマップ (IaC 管理済み vs. 取り込み対象)、判断履歴、
各判断の背後にある推論トレースを表示するので、ドリフト検出、docs PR、
アップグレード提案、ロールバックの承認ゲートを、ターミナルに触れることなく
ブラウザーから確認できます。

### 本物、でも復元可能

匿名の訪問者に開かれるのは、変更の影響範囲が限定的で、機械的に復元できる場合
だけです。そうでないものはゲートで守られます。

- **開放、でも復元可能:** Patch に脆弱な依存の修正を頼むと、本物の PR
  (`demo/upgrade-target/package.json` の 1 行) がマージされます。Anchor に
  ロールバックを頼むと、`payment-demo` のトラフィックが実際に以前のリビジョンへ
  移ります。訪問者が触れられる対象はすべてデモ用のフィクスチャであり、
  そのベースライン (`payment-demo` サービス、アップグレード用フィクスチャ、
  訪問者が開いた取り込み PR — これは適用されずにクローズされます) は
  オペレーターが公開期間の合間に復元します。
- **ゲート:** インフラ PR のマージには必ずオペレーターの本人確認が必要で、
  自由記述のインフラ作成は公開期間中オペレーター限定です (ワンクリックの「取り込む
  (Adopt)」経路は、常に影響のないゼロ変更インポートしか出さないため、開いた
  ままです)。マージ済みのインフラ PR は取り消せないので、匿名で行われることは
  ありません。

見えているのはモックアップでも性善説でもありません。実際の変更が反映され、
安全にリセットできない部分は、そもそも手が届かない部分になっています。
...
```bash
# Anchor (drift): Cloud Run の設定ドリフト
...
すべての DriftScribe ログ (推論の要約、ツール呼び出しイベント、
...
- [`workloads/`](workloads/) — エージェントチームごとのマニフェスト (`drift`、`upgrade`、`explore`、`provision`): システムプロンプト、コントラクト、ツール/ワーカー/アクション一覧
...
マルチテナントの薄い外殻よりも、「未定義のリソースを見つける → IaC PR を提案 →
人間が承認 → 適用」というループをエンドツーエンドで安全に動かしきることを優先
しました。単一テナントだからこそ、すべてのインフラ変更は人間の承認ゲートを通り、
ワーカー同士はサービスアカウントで相互認証し、`tofu-apply` ワーカーは自身の
イメージに焼き込まれたハッシュと一致する IaC のプランしか実行しません。
...
**より深いドリフト検知。** 現在のインフラマップが見つけられるのは、IaC 側に定義が
ないリソースです。定義そのものが無い以上ずれようがないため、UI ではドリフトとは
呼ばず、そのうち実際に取り込めるものを取り込み対象と呼んでいます。まだ見えないのは
本来の意味での「属性レベル」のドリフトで、管理下のリソースが定義と同一性では一致
したまま、実環境の設定だけが静かに乖離しているケースです (コンソールからのライフ
サイクルルール変更や、環境変数の書き換えなど)。設計済みの次のステップは、`tofu-apply`
ワーカーが毎回の適用前に実行している非破壊の `tofu plan -refresh-only` ゲートを
流用した定期チェックです。検知結果はインフラパネルに表示され、Provision が
調査するための証拠になります。あくまで検知のみで自動修復は行わず、人間の承認
ゲートはそのまま維持します。設計判断とフェーズ分割は
...
- **Infra-IaC エージェント** — プロジェクト全体のインベントリリーダー
  ...
  承認の背後にあるゲート付き `tofu-apply` ワーカー (稼働中のインフラに対して
...
- **オペレーター UI** — Svelte + Vite SPA として再構築され、サイトルート `/` で
  配信されます (オペレータートークンが必要)。実環境のインフラリソースマップ
  (IaC 管理済み vs. 取り込み対象) と、判断ごとのトレース + 環境変数の差分ビューを備えます。
