...
- **開放、自己回復:** Patch に脆弱な依存の修正を頼むと、本物の PR
  (`demo/upgrade-target/package.json` の 1 行) がマージされます。Anchor に
  ロールバックを頼むと、`payment-demo` のトラフィックが実際に以前のリビジョンへ
  移ります。スケジュール実行のワークフロー
  ([`demo-reset.yml`](.github/workflows/demo-reset.yml)) が両方のベースラインを
  復元します。サービスは 2 時間ごと、アップグレード用のフィクスチャは修正されて
  から数時間以内に復元されます。
...
ドリフト、つまり IaC に宣言されていないリソースの存在です。まだ見えないのは
「属性レベル」のドリフトで、管理下のリソースが宣言と同一性では一致したまま、
ライブ設定だけが静かに乖離しているケースです (コンソールからのライフサイクル
ルール変更や、環境変数の書き換えなど)。設計済みの次のステップは、`tofu-apply`
ワーカーが毎回の適用前に実行している非破壊の `tofu plan -refresh-only` ゲートを
流用した定期チェックです。検知結果はインフラパネルに表示され、Provision クルーが
