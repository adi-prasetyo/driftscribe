...
自律的に動作します。監視対象の `payment-demo` Cloud Run サービスの config 変更に反応するライブ Eventarc トリガーが組み込まれており（ポーリングループではなくイベント駆動）、チャットから呼び出す必要はありません。
...
**「ドリフト」には 2 つの意味があります。** Anchor のドリフトは config ドリフトです。`payment-demo` サービス 1 つのライブ環境変数を ops コントラクトと突合し、Eventarc が数秒以内に検知します。リソースマップのドリフトは別物で、`iac/` に記述されていないプロジェクト内の実リソースを指します。これは `infra-reader` ワーカーが Cloud Asset Inventory 経由で検出し、Anchor ではなく IaC への取り込み (Provision がインポート PR を作成) で解消します。Cloud Asset Inventory は結果整合性のインデックスで、リソースマップはその上にキャッシュされるため、手動で作成したリソースがマップに現れるまで数分かかります。これは検出のレイテンシであり、不具合ではありません。
...
- **ゲート:** インフラ PR のマージには必ずオペレーターの本人確認が必要で、
  自由記述のインフラ作成は公開期間中オペレーター限定です (ワンクリックの「採用
