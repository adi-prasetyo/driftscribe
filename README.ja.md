...
**「ドリフト」には 2 つの意味があります。** Anchor のドリフトは config ドリフトです。`payment-demo` サービス 1 つのライブ環境変数を ops コントラクトと突合し、Eventarc が数秒以内に検知します。リソースマップのドリフトは別物で、`iac/` に記述されていないプロジェクト内の実リソースを指します。これは `infra-reader` ワーカーが Cloud Asset Inventory 経由で検出し、Anchor ではなく IaC への取り込み (Provision がインポート PR を作成) で解消します。Cloud Asset Inventory は結果整合性のインデックスで、リソースマップはその上にキャッシュされるため、手動で作成したリソースがマップに現れるまで数分 (バケットはさらに長め) かかります。これは検出のレイテンシであり、不具合ではありません。
