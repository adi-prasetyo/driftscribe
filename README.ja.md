...
**「ドリフト」が指すのは設定ドリフトだけです。** Anchor のドリフトは、`payment-demo` サービス 1 つを対象に実環境の環境変数を ops コントラクトと突合するもので、Eventarc が数秒以内に検知します。ops コントラクトで宣言されていたものが実際にずれた、という意味でのドリフトです。リソースマップが答えるのは別の問いなので、言葉も分けています。`iac/` に記述されていないプロジェクト内の実リソースは、そもそも IaC に定義されたことがなく、ずれようがありません。そのため UI はこれをドリフトとは呼ばず、そのうち取り込める型のものを**取り込み対象**と呼んでいます。これは `infra-reader` ワーカーが Cloud Asset Inventory 経由で検出し、Anchor ではなく IaC 管理への取り込み (Provision がインポート PR を作成) で解消します。Cloud Asset Inventory は結果整合性のインデックスで、リソースマップはその上にキャッシュされるため、手動で作成したリソースがマップに現れるまで数分かかります。これは検出のレイテンシであり、不具合ではありません。
...
呼ばず、そのうち取り込める型のものを取り込み対象と呼んでいます。まだ見えないのは
