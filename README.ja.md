...
**「ドリフト」が指すのは config ドリフトだけです。** Anchor のドリフトは、`payment-demo` サービス 1 つを対象に実環境の環境変数を ops コントラクトと突合するもので、Eventarc が数秒以内に検知します。宣言されていたものが実際にずれた、という意味でのドリフトです。リソースマップが答えるのは別の問いなので、言葉も分けています。`iac/` に記述されていないプロジェクト内の実リソースは、そもそも宣言されたことがなく、ずれてもいません。そのため UI は、取り込めるものを**取り込み対象**と呼び、ドリフトとは呼びません。これは `infra-reader` ワーカーが Cloud Asset Inventory 経由で検出し、Anchor ではなく IaC 管理への取り込み (Provision がインポート PR を作成) で解消します。Cloud Asset Inventory は結果整合性のインデックスで、リソースマップはその上にキャッシュされるため、手動で作成したリソースがマップに現れるまで数分かかります。これは検出のレイテンシであり、不具合ではありません。
...
**より深いドリフト検知。** 現在のインフラマップが検知するのは「無い」ことです。
つまり、プロジェクトに実在するのに IaC に定義されていないリソースで、宣言が
そもそも無い以上ずれようがないため、UI ではドリフトではなく取り込み対象と
呼んでいます。まだ見えないのは本来の意味での「属性レベル」のドリフトで、管理下の
リソースが定義と同一性では一致したまま、実環境の設定だけが静かに乖離している
ケースです (コンソールからのライフサイクルルール変更や、環境変数の書き換えなど)。設計済みの次のステップは、`tofu-apply`
...
- **オペレーター UI** — Svelte + Vite SPA として再構築され、サイトルート `/` で
  ...
  (IaC 管理済み vs. ドリフト) と、判断ごとのトレース + 環境変数の差分ビューを備えます。
