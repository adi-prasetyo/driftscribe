...
オペレーター UI は、判断履歴と並べて実環境のインフラリソースマップ (IaC 管理済み vs. ドリフト) を表示します。

**「ドリフト」には 2 つの意味があります。** Anchor のドリフトは config ドリフトです。`payment-demo` サービス 1 つを対象に、実環境の環境変数を ops コントラクトと突合し、Eventarc が数秒以内に検知します。リソースマップのドリフトは別物で、`iac/` に記述されていないプロジェクト内の実リソースを指します。これは `infra-reader` ワーカーが Cloud Asset Inventory 経由で検出し、Anchor ではなく IaC 管理への取り込み (Provision がインポート PR を作成) で解消します。Cloud Asset Inventory は結果整合性のインデックスで、リソースマップはその上にキャッシュされるため、手動で作成したリソースがマップに現れるまで数分かかります。これは検出のレイテンシであり、不具合ではありません。
...
UI です。インフラのリソースマップ (IaC 管理済み vs. ドリフト)、判断履歴、
...
**より深いドリフト検知。** 現在のインフラマップが検知するのは「存在レベル」の
ドリフト、つまり IaC に定義されていないリソースの存在です。まだ見えないのは
「属性レベル」のドリフトで、管理下のリソースが定義と同一性では一致したまま、
実環境の設定だけが静かに乖離しているケースです (コンソールからのライフサイクル
ルール変更や、環境変数の書き換えなど)。設計済みの次のステップは、`tofu-apply`
