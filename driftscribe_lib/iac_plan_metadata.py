"""Canonical ``c2.v1`` plan-builder metadata schema (lib-owned).

This is the single source of truth for the C2 plan artifact ``metadata.json``
schema + its validators. It lives in ``driftscribe_lib`` (not ``tools``) so that
BOTH consumers can share one definition along the established ``tools -> lib``
dependency direction:

- ``tools/iac_plan_metadata.py`` — the C2 plan-builder CLI (re-exports from here
  and keeps the ``python -m`` entrypoint);
- ``driftscribe_lib/approvals.py`` (Phase C3) — re-validates operator-supplied
  metadata before signing a plan-bound approval, and the future C4 ``tofu-apply``
  worker re-validates fetched metadata at apply time. Worker containers ship
  ``driftscribe_lib/`` but NOT ``tools/`` (and ``tools/`` is not an installed
  package), so the schema MUST be importable from the lib for the worker to use.

The metadata record is the input contract for the C3 plan-approval schema
(docs/plans/2026-05-29-infra-iac-phase-c3-plan-approval.md) and was originally
specified in docs/plans/2026-05-28-infra-iac-phase-c2-plan-builder.md §3 — DO NOT
rename a field without updating C3.

Determinism: every public function here is a pure function of its arguments.
``serialize_metadata`` round-trips byte-identically given the same input
(sort_keys + fixed indent + no trailing whitespace).

Validation: ``build_metadata`` validates input shapes and raises ``ValueError``
on malformed inputs; it NEVER returns a partial / half-validated record.
"""
