...
4. **Frontend in-flight coalescing** (Codex must-fix 1). `InfraDiagram.refresh()` gains a component-instance flag: if a refresh is already in flight, a new trigger returns immediately (coalesce — the pending response is at most seconds stale, and the panel is advisory).
   ...
   - **Must not regress PR #99 (last-applied-wins):** the guard suppresses request *starts*, upstream of the response-application logic, which stays byte-identical (defense-in-depth — two in-flight responses can no longer arise from the component's own triggers, but the application policy remains correct if one ever does). No livelock is possible: the in-flight fetch always completes and (as the only outstanding run) always applies; the next trigger after completion fetches fresh. The skipped ladder rung is covered by the later rungs (10s rung skipped while the 0s fetch runs → 30s/60s rungs still ride out CAI lag).
   - **The PR #99 test pair must be RECAST, not kept** (Codex 019eba89, round 2): `InfraDiagram.test.ts:816` `mountWithTwoInFlight()` and its two tests (`:827` "applies an older 200…", `:842` "never lets an out-of-order older response…") explicitly wait for `pending.length === 2` — a state coalescing makes unreachable, so they would time out, not pass. They are replaced by tests proving the no-livelock property under the new request-start policy (Task 3); the describe-block comment narrating the livelock (`:789-799`) gains a line saying coalescing now prevents the stacking and last-applied-wins remains as the application policy.
...
### Task 3: frontend in-flight coalescing (TDD) — replaces the two-in-flight livelock tests

**Files:** Modify `frontend/src/components/InfraDiagram.svelte:197`, `frontend/tests/unit/InfraDiagram.test.ts:789-851`.

1. Write the failing tests in `InfraDiagram.test.ts`, REPLACING `mountWithTwoInFlight()` (`:816`) and the two tests at `:827`/`:842` (they wait for `pending.length === 2`, which coalescing makes unreachable — they'd time out). Reuse the existing `deferredCall()` helper. The recast pair proves no-livelock under the new request-start policy:
   - **"coalesces a trigger while a fetch is in flight — and the pending fetch still applies"**: mount (fetch #1 in flight, `pending.length === 1`), expand-toggle (the trigger that previously started fetch #2), assert `pending.length` STAYS 1 (this is the assertion that FAILS pre-implementation, observing 2); resolve #1 → the panel applies its data (coverage count renders — the livelock guard).
   - **"a trigger after completion fetches fresh and supersedes"**: continue — after #1 applied, fire a new trigger (re-toggle or focus), assert a second request starts, resolve it with different totals, assert the newer data renders.
   - Update the describe-block comment (`:789-799`): the boot-stampede narration stays; add that request starts now coalesce (one in-flight per panel) and last-applied-wins remains the application policy.
   Run `cd frontend && npm run test:unit` → recast test #1 FAILS (two pending observed).
2. Implement: a `let refreshInFlight = false;` component flag; first lines of `refresh()` become `if (refreshInFlight) return; refreshInFlight = true;` with `refreshInFlight = false;` added in the existing `finally`. A short comment stating the constraint: one in-flight graph fetch per panel — the describe budget is now 90s and the coordinator runs concurrency=2, so stacked polls/ladder rungs must coalesce; response-application logic (PR #99 last-applied-wins) unchanged as defense-in-depth.
3. Run: `npm run test:unit` PASS; `npm run check` clean. Full backend suite `.venv/bin/pytest -q` (unchanged, for the PR gate).
