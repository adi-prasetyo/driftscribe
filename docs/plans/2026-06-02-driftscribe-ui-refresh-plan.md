...
```
P0 contract patches ─> P1 Scaffold ─┬─> P2 lib/* (+ vitest)  ─┐
                                     └─> P3 styles/tokens       ├─> P4 components ─> P5 FastAPI shell+static ─> P5b approval restyle ─> P6 test migration ─> P7 build/CI/Docker ─> P8 verify+PR
                                                                ┘
```
P0 are small server/config patches applied before fan-out (Codex review,
2026-06-02). P2 modules are independent of each other (parallel). P4 components
depend on P2 interfaces (fixed in §3), so they parallelize once P2 interfaces
exist (tests may still be red). P5 (shell + static plumbing) is split from P5b
(approval markup restyle) so the risky shell work isn't coupled to approval
churn. P5/P5b/P6/P7 are sequential. P8 is the gate.

## 2a. P0 — Contract patches before fan-out (from Codex review)

Apply these first; they are small, server/config-side, and unblock the rest.

1. **Lockfile vs `npm ci`.** Global `.gitignore:17` ignores `package-lock.json`.
   Add `!frontend/package-lock.json` and **commit** `frontend/package-lock.json`
   (generated by P1's `npm install`). Keep `npm ci` in Docker/CI.
2. **IaC approval CSP.** `_apply_iac_csp` (agent/main.py ~239) sets
   `style-src 'unsafe-inline'`, which would **block** an external
   `<link rel="stylesheet">`. Change to
   `default-src 'none'; style-src 'self'; form-action 'self'; base-uri 'none';
   frame-ancestors 'none'`. Update the CSP assertion in
   `tests/integration/test_iac_approval_get.py` (~line 313). Verify the rollback
   `/approvals/{id}` page has no conflicting CSP (it currently ships no CSP →
   external CSS is fine; confirm in P5b).
3. **Manifest lookup rules.** The shell + approval pages resolve assets by Vite
   **manifest key `src/main.ts`**: use `manifest["src/main.ts"].file` for the JS
   and `manifest["src/main.ts"].css[]` for the stylesheet(s). Do **not** depend
   on specific output filenames. `_vite_asset()` is **lazy per request** and
   caches only on a successful manifest read (never caches the dev fallback), so
   a build later in the same process is picked up. Dev fallback (manifest
   absent) → `/static/transparency.js` + `/static/driftscribe.css` so the shell
   route still returns 200 in the pure-Python CI job.
4. **`mcp_call` is a real event kind** (NOT tool-prefix routing — that part of
   §3 below is corrected). See §3 `lib/timeline.ts` and the App backfill rule.
...
### `lib/sse.ts`
```ts
// NOTE: the SSE stream carries these kinds. `mcp_call` is NOT streamed — it is
// a side-channel log kind that appears only in GET /trace (see timeline.ts +
// App backfill rule). So ChatEvent (stream) excludes mcp_call; TraceEvent
// (timeline.ts) includes it.
export type ChatEvent =
  ...
  | { event: 'tool_result'; trace_id: string; workload: string; tool_name: string; result_preview: string; result_ok: boolean; latency_ms?: number|null }
...
// TraceEvent is the superset (stream events + the trace-only `mcp_call`).
// mcp_call fields: { event:'mcp_call', trace_id, workload, mcp_tool?, mcp_server?,
//   latency_ms?, doc_count?, ... }.
export interface TraceEvent extends Record<string, unknown> {
  ...
  mcp_tool?: string; mcp_server?: string;  // mcp_call
}
// CORRECTED binning (matches the existing renderer, transparency.html:1285-1296):
...
//   final_response                   → skipped (rendered in #final-response-card)
//   unknown                          → dropped
// There is NO tool-name-prefix routing to mcp. MCP_TOOL_PREFIXES is REMOVED.
export function groupOf(e: TraceEvent): GroupKey | null;
export function subKey(e: TraceEvent): string;   // tool_name | mcp_tool||mcp_server
export function groupEvents(events: TraceEvent[]): Record<GroupKey, TraceEvent[]>;
export function pairToolEvents(events: TraceEvent[]): Array<{call?: TraceEvent; result?: TraceEvent}>;
...
### `lib/labels.ts`
```ts
// Friendly worker/tool/MCP labels (preserve exact strings the old renderer used).
export const WORKER_LABELS: Record<string, string>; // keyed by tool __name__ / mcp key
export function workerLabel(key: string): string;    // falls back to raw key
...
// resolve to window.location.origin AND whose path starts with "/approvals/";
// reject off-origin / non-http(s) schemes. Returns a RELATIVE href
// (pathname+search) on success — matching the existing renderer
// (transparency.html:1010) — or null if rejected. Components MUST route every
// approval link through this; a vitest+smoke test asserts ApprovalCta and
// DecisionsRail actually use it (not just that the fn exists).
...
- `App.svelte`: wires SSE submit (live) + **one post-`done` `/trace` backfill**
  (reconciles event ordering AND pulls side-channel `mcp_call` events not carried
  on the stream — transparency.html:1616) + poll fallback + historical replay
  (fetch `/trace` once) + decisions load + auth flow; the **settle** animation
  (transition on event rows; flip on reflow) gated by reduced-motion.
- **Verify:** `npm run check` clean; `npm run build` succeeds.

### P5 — FastAPI shell + static plumbing (sequential, 1 agent)
...
- `agent/main.py`: add `_vite_asset()` manifest reader per P0.3 (lazy, cache on
  success only, manifest key `src/main.ts` → `.file` + `.css[]`; dev fallback),
  mount `StaticFiles(directory=.../static, check_dir=False)` at `/static`, pass
  asset URLs into the shell context. Keep `Cache-Control: no-store`,
  unauthenticated GET. Expose a helper the approval routes also use for `ds_css`.
- **Verify:** app imports cleanly; `GET /ui/transparency` 200 with `id="app"`
  both with a built manifest and without (fallback). `/static` mount present.

### P5b — Approval restyle (sequential, 1 agent, after P5)
- Apply the P0.2 CSP change in `_apply_iac_csp` (`style-src 'self'`).
- Restyle `approval.html` + `iac_approval.html`: replace inline `<style>` with a
  `<link rel="stylesheet" href="{{ ds_css }}">` (manifest-resolved via the P5
  helper) + new `ds-*` classes. **Keep every Jinja block, the
  `<form method="post">`, hidden `t`/`form_token` fields with their
  `data-testid`, and all conditionals unchanged.** Cover BOTH GET render and
  POST-result render (the result/outcome banners use the same templates with
  extra context — agent/main.py:3214 / iac POST).
- **Verify:** `uv run pytest tests/integration/test_approvals.py
  tests/integration/test_iac_approval_get.py tests/integration/test_rollback_e2e.py -q`
  green (form/testids/CSP/POST banners intact).
...
- `.github/workflows/ci.yml`: add `frontend` job (node 24 → npm ci → build →
  check → test:unit) and a **REQUIRED** `ui-smoke` job (python+node → npm ci →
  build → boot uvicorn → `npm run test:smoke`). NOT continue-on-error — this is
  the pre-merge stand-in for the dispatch-only cloud e2e. Leave `lint-test`
  unchanged.
- `frontend/tests/smoke/transparency.smoke.ts` + a tiny runner that starts
  `uvicorn agent.main:app` (built assets present) and points Playwright at it
  with `page.route` mocks for `/chat` (canned SSE), `/decisions`, `/trace`.
  Assert (Codex expanded list): built static assets load with **no 404**; the 7
  testids; three-group `.open` reveals `[data-group]`; the app sends
  `Accept: text/event-stream`; `X-DriftScribe-Token` is sent from sessionStorage;
  401/403 clears token and shows AuthPanel; canned SSE `meta`/data-events/`done`
  renders the final response; a `/trace` backfill carrying `mcp_call` populates
  the MCP group; a malicious off-origin/`javascript:` approval URL renders **no
  link**; historical mode + "← new chat". Factor shared selector/testid
  constants so the smoke and the deployed `tests/e2e/ui` spec don't drift.
