...
**Goal:** Stop every adoption surface (panel adopt list, Start-here pick, onboarding-tour step 4, chat prompts) from suggesting DriftScribe's OWN control-plane resources — whose adoption the denylist is guaranteed to refuse — by flagging those nodes server-side with the denylist's own identity constants.

**Architecture:** `build_graph` adds an additive, only-when-true `control_plane: true` flag to nodes whose (asset type, name) matches a denylist control-plane identity rule; the three client adopt surfaces suppress the CTA for flagged nodes with an honest note; the two workload prompts gain a pinned canonical sentence so chat refuses up front. No denylist change, no inventory change — coordinator rebake only.
...
5. **Safety framing is CORRECT here.** Item 10's rule (confidence-framing, never safety) governs the adoption-ORDER copy. This note states what the always-on gate actually does — like the capability card's rule descriptions, it is a gate fact, and tests pin it as such.
6. **No denylist / inventory / worker changes.** `infra_graph.py` already imports from `iac_plan_denylist`; both ship in the coordinator image. **Coordinator rebake only** — no tofu-editor, tofu-apply, or infra-reader rebake.
...
(`TOPIC_TYPE` already exists alongside `RUN_TYPE`/`BUCKET_TYPE`/`SECRET_TYPE`; if the constant is named differently, use the file's existing Pub/Sub topic constant. `ADOPTABLE_ASSET_TYPES` and `build_graph` are already imported.)
...
# DriftScribe's OWN -tofu-artifacts bucket). The denylist hard-refuses any
# plan touching a control-plane identity — IMPORTS INCLUDED (evaluate runs the
# identity rules `if _is_mutation(actions) or importing is not None`) — so an
# Adopt button on such a node is a guaranteed dead end at C2 evaluation. Nodes
...
# capability card's rule descriptions.
ADOPTION_CONTROL_PLANE_NOTE = (
    ...
    "always-on denylist refuses any plan that touches them, imports included."
)
```
...
- DriftScribe's own control-plane resources — its Cloud Run services and the
  ...
  denylist refuses any plan that touches them, imports included. If the
  operator asks to adopt one, say so plainly and do not call
  provision_propose_adoption for it.
```
...
- DriftScribe's own control-plane resources — its Cloud Run services and the
  -tofu-state / -tofu-artifacts buckets — cannot be adopted: the always-on
  denylist refuses any plan that touches them, imports included. Never
  suggest one as a first adoption.
```
...
### Task 3: frontend lib — node type + `adoptRows`
...
### Task 4: InfraDiagram — suppress button, honest note, Start-here fix
...
it('control-plane row shows the denylist note instead of an Adopt button', async () => {
  ...
  expect(note.textContent).toContain("DriftScribe's own infrastructure");
...
```svelte
                {:else if row.controlPlane}
                  <span class="ds-subtle infra-adopt__muted" data-testid="adopt-control-plane"
                    >DriftScribe's own infrastructure — the always-on denylist blocks changes
...
### Task 5: tour step 4 — skip control-plane nodes + honest fallback
...
it('all-control-plane estate gets the honest denylist line, not the unnamed line', () => {
  ...
  if (s.kind === 'none') {
    expect(s.line).toContain("DriftScribe's own infrastructure");
  ...
  if (unmanagedShown.length > 0 && nonControlPlane.length === 0) {
    return {
      ...
      line:
        ...
        "DriftScribe's own infrastructure, which the always-on denylist " +
        'blocks the agent from changing — adoption included. The ' +
        'Infrastructure panel shows everything that is there.',
    };
  }
