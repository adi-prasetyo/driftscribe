...
**Status:** IN PROGRESS — reviewed by Codex (thread `019e78c2`). **Phases 1–6 EXECUTED live (1–4 + spikes 2026-05-30; Phases 5–6 on 2026-05-31)** (see EXECUTION LOG below); open-decisions #1 = **`run.admin`** and #5 = **retire the spikes** RESOLVED. The build SA `cloudbuild-deploy-sa@` is PROVEN GREEN across every permission class — including a **post-editor-strip smoke** (`tofu-apply.yaml` build `1dfea533`, all 4 steps SUCCESS AS the build SA *after* the compute SA's `editor` was gone). **The compute SA now holds ZERO project roles** (roleless), empty resource policy, no user keys — but is **NOT yet disabled**. **Only Phase 7 remains** (disable the SA + patch latent regression sources + prune legacy `@cloudbuild` bindings). Codex reviewed Phase 6 (`019e78c2`): "looks clean … no gap I'd call a blocker."
**Date:** 2026-05-30 (Phases 5–6 executed 2026-05-31)
...
**Held (not executed):** **Phase 7** — `disable` (not delete) the now-roleless compute SA + close the latent regression sources. The compute SA is roleless, consumer-free, and the build SA is proven self-sufficient (incl. post-strip), so disabling is low-risk + instantly reversible. Codex-recommended Phase-7 ordering (`019e78c2`): (1) patch the code/script reintroduction sources first (`setup_prod_project.sh:303`, `setup_e2e_project.sh:338-339`, any docs defaulting a runtime SA to `PROJECT_NUMBER-compute@`), (2) prune the live legacy `@cloudbuild` actAs bindings (recon found them still on 8/10 runtime SAs) once confirmed unused, (3) `disable` the SA **last**, then a negative-reference check + one pinned-build smoke / config validation — this keeps the fastest rollback (re-grant only, no `enable`) available while the reintroduction sources are removed.
...
### Phase 7 — DISABLE (not delete) + close the regression sources — HELD (separate operator go)
...
## Latent regression sources to fix (don't let bootstrap undo the work)
- `infra/cloudbuild.yaml:174` — `payment-demo` deploy: `--allow-unauthenticated`, **no** `--service-account` → would revert payment-demo onto the compute SA + break tofu zero-diff. Pin `payment-demo-runtime@`.
- `infra/cloudbuild.yaml:5` + `infra/cloudbuild.coordinator-update.yaml:8` — stale "Option C / default compute SA" comments (live coordinator runs as `driftscribe-agent@`).
- `setup_secrets.sh:117` + actAs loop (`:158`) — targets the wrong (legacy `@cloudbuild`) SA and misses `upgrade-reader`/`upgrade-docs`/`infra-reader`/`payment-demo-runtime`.
- `setup_prod_project.sh:303`, `setup_e2e_project.sh:338-339` — runtime-SA defaults to `PROJECT_NUMBER-compute@`.
