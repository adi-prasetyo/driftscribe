...
| H5 | Unknown resource types say "no estimate available" (`kind="unknown"`, counted in the headline); unrecognized attr values show NO rate — never invent (e.g. an unknown storage class stays `kind="usage"` with a generic "billed per GiB stored" note, never priced as Standard) | unknown types → `kind="unknown"`; `_est_bucket.cls()` returns None for present-but-unrecognized classes → rate-free note |
