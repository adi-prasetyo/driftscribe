...
**Workload 1 (Drift, 5 beats):** beat-a establishes a clean baseline that resolves to `no_op`; beat-b introduces a deliberate drift and turns it into a `drift_issue`; beat-c is the ADK reasoning beat where the agent explains the cause; beat-d previews a docs PR from the docs worker; beat-e exercises the rollback worker through the HITL approval gate.
