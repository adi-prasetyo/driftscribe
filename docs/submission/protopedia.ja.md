...
DriftScribe は、Cloud Run 上で安全に AI 主導の DevOps を行うためのマルチエージェント・フレームワークです。単一のワークロード対応コーディネーター (Google ADK + Vertex AI Gemini 3.5 Flash) がオペレーターのリクエストをワークロード別のエージェントへルーティングし、各エージェントは自分のワークロードに許可されたツールしか見えません。本日時点で 4 つのクルーが動いています: **Anchor** はライブの Cloud Run サービス (`payment-demo`) を ops コントラクトと照合し、`no_op` / `docs_pr` / `drift_issue` / `rollback` / `escalation` から選択。**Patch** は npm `package.json` を GitHub Advisory DB と照合し、`no_op` / `docs_pr` / `upgrade_pr` / `escalation` から選択。**Explore** はライブインフラとコードを横断する読み取り専用の調査役で、ミューテーションツールをゼロしか持ちません。**Provision** は OpenTofu の変更を書き、ゲート付き・HMAC 署名付きの apply パイプラインを通る `iac/` のみの PR を 1 つ開きます。Anchor は自律的（Cloud Run config 変更に反応するライブ Eventarc トリガー）に、残り 3 つはチャットからのオンデマンドで動作します。推論は Google の Developer Knowledge MCP (コーディネーターにのみアタッチ) によって裏付けられます。破壊的経路は二重にガードされています。rollback は HMAC 署名付きの 1 回限りの HITL 承認、upgrade-PR は LLM 直後の決定論的バリデーター (semver、GHSA URL 形式、パス正規表現) が fail-closed で守ります。エージェントが提案し、オペレーター (またはバリデーター) が裁定し、ワーカー境界が「提案」を安全に公開できるようにします。
...
- LLM: Gemini 3.5 Flash (Vertex AI、`thinking_level=high`、`GOOGLE_CLOUD_LOCATION=global`。この Gemini 3 モデルが提供されるエンドポイントであり、コーディネーターが推論サマリーを受け取るため。Cloud Run 自体は asia-northeast1)
...
DriftScribe のクルーは、1 つのクラウドエステートを取り巻くループを形づくります。Provision が新しいインフラを立ち上げ、Anchor が唯一の自律トリガーでライブのインフラを見守り、Patch が最新の状態を保ち、Explore がそれを説明します。デモはこの流れに沿って進みます。
...
**現在のスコープ（意図的な単一テナント構成）。** DriftScribe は 1 つの GitHub リポジトリと 1 つの Google Cloud プロジェクトに紐づいて動作します。これは意図的な判断です。マルチテナントの薄い外殻よりも、「未定義のリソースを見つける → IaC PR を提案 → 人間が承認 → 適用」というループをエンドツーエンドで安全に動かしきることを優先しました。上述の保証（人間の承認ゲート、ワーカー間のサービスアカウントによる相互認証、自身のイメージに焼き込まれたハッシュと一致する IaC のプランしか適用しないワーカー）は、単一テナントだからこそ成立します。
