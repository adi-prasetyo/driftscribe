...
**伝えるべきストーリー。** DriftScribe では、4 つのエージェントチームが 1 つのクラウドエステートを継続的に見守り、全体で 1 つのループを形成します。Provision が新しいインフラを立ち上げ、Anchor が稼働中のインフラを自律的に見守り、Patch が最新の状態を保ち、Explore がそれを説明します。以下のスクリプト化された beat は、そのうち 2 つのエージェントチーム（Anchor のドリフト検知、Patch のアップグレード）をたどります。Provision と Explore はチャットから対話的に見せます。審査員への一貫したメッセージは「一度プロビジョニングすれば、あとは Anchor が見守り続ける」です。
...
> **録画前に:** 手動ディスパッチの E2E ワークフローを起動し、グリーンになるまで待ちます。
> `gh workflow run e2e.yml` を実行し、続けて `gh run watch`。`e2e` GitHub
> Environment のレビュアー承認が必要です。E2E 実行はデモ経路が無傷であることを
> 確認する fail-fast シグナル — 録画前に毎回これを回しておくことで、IAM / MCP /
> worker 境界のリグレッションを (カメラの前に出る前に) 捕まえられます。
...
| ----- | ------------------------------------- | --------------------------------- | -------------------------------------------------------------------------------------------------- |
| 0:00  | `./scripts/demo.sh beat-a`            | "Read" 経路までスクロール          | 「DriftScribe は稼働中の Cloud Run サービスを ops コントラクトと照合します。ベースラインチェックは no_op を返します」 |
...
Phase 19.B はコーディネーターのルート `/` にオペレーター向けの推論タイムライン UI を追加します。各 `/chat` 呼び出しの最終応答は即座に表示され、その後 3 つの推論グループ (Coordinator / Tools & workers / MCP) が Cloud Logging のインジェスト (約 15 秒のラグ) に合わせて埋まっていきます。右ペインには過去の判断履歴が並び、pending な rollback はインラインで承認ページへ飛べます。
...
スタブ fetcher (`agent/trace_fetcher.py`) は合成タイムラインを返すので、GCP 認証なしで描画経路を検証できます。`Cache-Control: no-store` ヘッダはすべてのオペレーターサーフェス (`/`、`/trace`、`/decisions`) に設定されています — `curl -i http://localhost:8080/ | grep -i cache-control` で確認できます。

### ウォークスルー beat

| ステップ | 操作                                                                                                | 期待結果 (タイミング上限つき)                                                                                                                |
| -------- | --------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| 1        | ブラウザで `https://<coordinator-url>/` を開く。                                                    | ページが読み込まれ、トークン入力モーダルが表示される (`sessionStorage` にトークンがまだない)。                                              |
