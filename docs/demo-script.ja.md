...
**伝えるべきストーリー。** DriftScribe は 1 つのクラウドエステートを継続的に見守る小さなクルーで、全体はループとして動きます。Provision が新しいインフラを立ち上げ、Anchor がライブのインフラを自律的に見守り、Patch が最新の状態を保ち、Explore がそれを説明します。以下のスクリプト化された beat は、そのうち 2 つのクルー（Anchor のドリフト検知、Patch のアップグレード）をたどります。Provision と Explore はチャットから対話的に見せます。審査員への一貫したメッセージは「一度プロビジョニングすれば、あとは Anchor が見守り続ける」です。
