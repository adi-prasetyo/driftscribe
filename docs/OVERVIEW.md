...
1. **Anchor**: the only one that runs **autonomously**. A live Eventarc
   ...
   a docs PR, file a drift issue, or **roll back** the service. A rollback always requires a
...
4. **Provision**: an on-demand, **chat-only** workload that can
   **author OpenTofu/Terraform changes**, but only as a *proposal*: its one mutation tool writes
   ...
   a plan-bound, HMAC-signed operator approval. It runs in the `tofu-apply` worker, the *only* thing
   that runs `tofu apply` against live infra, and the chat agent cannot invoke that worker
   directly.
