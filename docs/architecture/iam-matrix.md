...
> **Status:** Phase 11.4 — coordinator SA (`driftscribe-agent`, Phase 8) and the reader worker (`reader-agent-sa`, Phase 11.3) are live. The docs worker (`docs-agent-sa`, this phase) is provisioned via the operator command block in the 11.4 commit message. The remaining two worker SAs (rollback/notifier) are still TODO and listed here so the target state is reviewable now.
