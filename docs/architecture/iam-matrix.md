...
> **Status:** Phase 11.5 — coordinator SA (`driftscribe-agent`, Phase 8), the reader worker (`reader-agent-sa`, Phase 11.3), the docs worker (`docs-agent-sa`, Phase 11.4), and the rollback worker (`rollback-agent-sa`, this phase) are live. The rollback SA is provisioned via the operator command block in the 11.5 commit message. The remaining worker SA (notifier) is still TODO and listed here so the target state is reviewable now.
